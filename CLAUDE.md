...
- [x] 폴더구조 및 기본 파일 생성 (img/css/js, index.html) (2026-03-27)
...
| 컬럼 | 타입 | 기본값 | 설명 |
|------|------|--------|------|
| id | bigint | auto increment | 자동 증가 PK (목록 번호로 사용) |
| is_notice | boolean | false | 공지글 여부 (true 시 목록 상단 고정 + 공지 뱃지 표시) |
| title | text | - | 제목 |
| content | text | - | 내용 |
| author_name | text | - | 작성자 (이메일 @ 앞 아이디) |
| user_id | uuid | - | 작성자 Supabase Auth ID |
| image_url | text | null | 첨부 이미지 URL (없으면 null) |
| views | integer | 0 | 조회수 (상세 진입 시 자동 증가) |
| likes | integer | 0 | 좋아요 수 |
| created_at | timestamp | now() | 작성일 (자동 생성) |
